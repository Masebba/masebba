...
export function Dashboard() {
  ...
  },
  {
    ...
    value: services.filter((s) => s.isVisible).length.toString(),
    icon: SettingsIcon,
    link: '/admin/services'
  }];
  ...
  posts.slice(0, 2).forEach((post) => {
    recentActivity.push({
      id: `post-${post.id}`,
      action:
      post.status === 'published' ? 'Published blog post' : 'Draft saved',
      target: `"${post.title}"`,
      time: post.createdAt ?
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat) => {
          ...
          return (
            <Card
              key={stat.name}
              className="flex items-center p-6 cursor-pointer hover:border-primary transition-colors"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <Card padding="lg" className="lg:col-span-2">
          ...
          <p className="text-muted text-sm">
              No recent activity. Start by adding projects, blog posts, or
              services.
            </p>
          }
        </Card>
