...
export function Messages() {
  ...
  {
    header: 'Name',
    accessor: 'name' as keyof ContactMessage,
    className: 'font-medium text-main'
  },
  {
    header: 'Email',
    accessor: 'email' as keyof ContactMessage
  },
  {
    header: 'Status',
    cell: (item: ContactMessage) => <StatusBadge status={item.status} />
  },
  {
    header: 'Date',
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex bg-surface rounded-md p-1 border border-border">
          <button
            ...
            className={`px-3 py-1 text-sm font-medium rounded ${filter === 'all' ? 'bg-background shadow-sm text-main' : 'text-muted'}`}>
            
            All
          </button>
          <button
            onClick={() => setFilter('unread')}
            className={`px-3 py-1 text-sm font-medium rounded ${filter === 'unread' ? 'bg-background shadow-sm text-main' : 'text-muted'}`}>
            
            Unread
          </button>
          <button
            onClick={() => setFilter('read')}
            className={`px-3 py-1 text-sm font-medium rounded ${filter === 'read' ? 'bg-background shadow-sm text-main' : 'text-muted'}`}>
      ...
      <Modal
        ...
        <div className="space-y-6">
            ...
            <div>
              ...
              <div className="bg-surface p-4 rounded-lg text-main whitespace-pre-wrap">
