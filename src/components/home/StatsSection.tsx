...
function StatItem({ value, suffix, label }: { value: number; suffix: string; label: string }) {
  ...
  return (
    <div ref={ref} className="text-center">
      <div className="text-3xl md:text-5xl font-bold text-primary mb-2">{count}{suffix}</div>
      <div className="text-sm md:text-base font-medium text-muted uppercase tracking-wider">{label}</div>
    </div>
  );
}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-12">
            {Array.from({ length: 4 }).map((_, index) => (
              <div key={index} className="text-center space-y-2">
                <Skeleton variant="text" width="70%" height={42} className="mx-auto" />
